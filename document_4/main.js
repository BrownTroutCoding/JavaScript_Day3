...
form.addEventListener('submit', (event) => {
    ...
    // Check if event.path exists and has an element at index 0
    if (event.path && event.path[0]) {
      // Check if the element at index 0 has child elements at index 0 and 1
      if (event.path[0][0] && event.path[0][1]) {
    ...
    console.log(`This came from the query selector: ${first_name_input.value}, ${last_name_input.value}`);
  });
